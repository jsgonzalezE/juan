...
(function () {
  ...
  function coffeeName(sku) {
    var c = null;
    for (var i = 0; i < catalog.length; i++) if (catalog[i].sku === sku) { c = catalog[i]; break; }
    return c ? c.name : sku;
  }
  ...
  function showView(name) {
    ...
    else if (scanSession.mode) startCamera();
  ...
  var scanSession = {
    mode: null,          // null | 'order' | 'free'
    orderId: null,
    freeTally: {},       // sku -> {lbs, bags}
    undoStack: [],
    paused: false,
    lastSeen: {},        // payload -> ts del último frame en que se vio
  ...
  async function startCamera() {
    ...
    scanner = new QRScanner(video, {
      ...
      onStatus: function (st) {
        ...
        $('top-status').textContent = st.engine === 'nativo' ? 'escáner nativo ⚡' : 'escáner listo';
      }
    });
    try {
      await scanner.start();
  ...
  document.addEventListener('visibilitychange', function () {
    if (document.visibilityState === 'visible') {
      if (currentView === 'scan' && scanSession.mode) { startCamera(); }
    } else stopCamera();
  });
  ...
  function onDecode(raw) {
    if (scanSession.paused || !scanSession.mode) return;
  ...
  function ensureCoffee(sku) {
    for (var i = 0; i < catalog.length; i++) if (catalog[i].sku === sku) return;
  ...
  function logScan(sku, lbs, orderId, source) {
    scans.push({ ts: Date.now(), sku: sku, lbs: lbs, orderId: orderId, source: source || 'scan' });
    saveScans();
  }
  ...
  function undoLast() {
    ...
    if (!u) return;
    // quitar del log el último escaneo que coincida
  ...
  function renderScanLines() {
    ...
    if (scanSession.mode === 'free') {
      ...
      if (!skus.length) {
        host.innerHTML = '<p class="hint">Apunta la cámara al QR de cada bolsa. Aquí verás el conteo.</p>';
      } else {
        host.innerHTML = skus.map(function (sku) {
          var t = scanSession.freeTally[sku];
          return '<div class="line done"><div class="line-fill"></div><div class="line-row">' +
            '<span class="line-name">' + esc(coffeeName(sku)) + '</span>' +
            '<span class="line-progress">' + E.fmtLbs(t.lbs) + ' lb · ' + t.bags + ' bolsas</span>' +
            '</div></div>';
        }).join('');
      }
      $('scan-progress-pct').textContent = '';
    ...
    host.innerHTML = o.job.lines.map(function (l) {
      ...
      return '<div class="line' + (done ? ' done' : '') + '">' +
        '<div class="line-fill" style="width:' + pct + '%"></div>' +
        '<div class="line-row">' +
        '<span class="line-name">' + (done ? '✅ ' : '') + esc(l.name) + '</span>' +
        '<span class="line-progress">' + E.fmtLbs(l.doneLbs) + ' / ' + E.fmtLbs(l.needLbs) + ' lb</span>' +
        '</div>' +
        '<div class="line-bags">' + esc(bags) + '</div>' +
        '</div>';
    }).join('');
    var t = E.jobTotals(o.job);
    $('scan-progress-pct').textContent = t.needLbs ? Math.round((t.doneLbs / t.needLbs) * 100) + '%' : '';
  }
  ...
  function renderPendingChips() {
    ...
    if (!pend.length) {
      host.innerHTML = orders.length
        ? '<p class="hint">🎉 Todas las órdenes del día están listas.</p>'
        : '<p class="hint">No hay órdenes importadas. Ve a la pestaña Órdenes para pegarlas o subir el archivo.</p>';
      return;
    }
    host.innerHTML = pend.map(function (o) {
      return '<button class="order-chip status-' + o.status + '" data-id="' + esc(o.id) + '">' +
        '<span class="oc-id">' + (o.status === 'en-proceso' ? '⏳ ' : '') + esc(o.id) + '</span>' +
        '<span class="oc-sub">' + esc(orderSummaryText(o)) + '</span>' +
        '</button>';
    }).join('');
  ...
  function showComplete(o) {
    ...
    $('complete-box').textContent = box ? '📦 ' + box : '';
  ...
  function lbsChipsHtml(idPrefix) {
    return '<div class="big-lbs">' +
      ...
      'placeholder="Otras libras"><button id="' + idPrefix + '-custom-ok" class="secondary">Usar</button></div>';
  }

  // QR viejo con café conocido pero libras variables → preguntar libras (1 toque)
  function openLbsSheet(sku, raw) {
    openSheet(
      '<h3>' + esc(coffeeName(sku)) + ' — ¿de cuántas libras es la bolsa?</h3>' +
      lbsChipsHtml('lbs') +
      '<button id="lbs-cancel" class="secondary big">Cancelar</button>'
    );
    function pick(lbs) { closeSheet(); applyBag(sku, lbs, 'scan'); }
  ...
  // QR desconocido → aprenderlo una sola vez
  function openLegacySheet(raw) {
    ...
    openSheet(
      ...
      '<label style="margin-top:10px">¿De cuántas libras?</label>' +
      lbsChipsHtml('legacy') +
      '<label class="toggle"><input type="checkbox" id="legacy-varies"> Este QR se usa en varios tamaños ' +
      '(preguntar libras cada vez)</label>' +
      '<button id="legacy-cancel" class="secondary big">Cancelar (no contar)</button>' +
    ...
    if (scanSession.mode === 'order') {
      ...
      openSheet(
        ...
        '<div class="sheet-list">' + lines.map(function (l) {
          return '<button data-sku="' + esc(l.sku) + '">' + esc(l.name) +
            ' <small>faltan ' + E.fmtLbs(l.needLbs - l.doneLbs) + ' lb</small></button>';
        }).join('') + '</div>'
      );
      $('sheet').querySelectorAll('.sheet-list button').forEach(function (b) {
        b.addEventListener('click', function () {
          ...
          openSheet('<h3>' + esc(coffeeName(sku)) + ' — ¿bolsa de cuántas libras?</h3>' + lbsChipsHtml('man') +
            '<button id="man-cancel" class="secondary big">Cancelar</button>');
      ...
      openSheet(
        ...
        '<label style="margin-top:10px">Libras</label>' + lbsChipsHtml('man') +
        '<button id="man-cancel" class="secondary big">Cancelar</button>'
      );
      $('sheet').querySelectorAll('.big-lbs button').forEach(function (bb) {
  ...
  function renderOrders() {
    ...
    if (!orders.length) {
      sum.innerHTML = '';
      host.innerHTML = '<p class="hint">Sin órdenes. Pega el texto de las órdenes del día abajo, o súbelo como archivo.</p>';
  ...
  function renderReports() {
    ...
    if (!sum.perSku.length) {
      $('report-table').innerHTML = '<p class="hint">Sin escaneos en este periodo.</p>';
    } else {
      var rows = sum.perSku.map(function (p) {
        ...
        return '<tr><td>' + esc(coffeeName(p.sku)) + '</td>' +
  ...
  $('woo-file').addEventListener('change', function () {
    ...
    reader.onload = function () {
      ...
      res.items.forEach(function (it) {
        ...
        var key = it.product;
        if (!byMonth[mk].has(key)) byMonth[mk].set(key, { product: it.product, qty: 0 });
        byMonth[mk].get(key).qty += it.qty;
      });
  ...
  function renderWoo() {
    ...
    // guardar mapeos automáticos para no recalcular ni volver a adivinar
    var autoKeys = Object.keys(agg.autoMapped);
    if (autoKeys.length) {
      autoKeys.forEach(function (k) { wooMap[k] = agg.autoMapped[k]; });
      saveWooMap();
    }
    ...
    if (agg.unmatched.length) {
      un.innerHTML = '<p class="preview-warn">⚠ ' + agg.unmatched.length +
        ...
        agg.unmatched.map(function (u, idx) {
          var options = catalog.map(function (c) {
          ...
          return '<div class="preview-order" data-product="' + esc(u.product) + '">' +
            ...
            '<button class="primary um-save">✔</button>' +
            '</div></div>';
        }).join('');
    ...
    if (!cmp.length) {
      $('woo-compare').innerHTML = '<p class="hint">Sin datos para comparar en ' + wooSelectedMonth + '.</p>';
      $('btn-export-compare').hidden = true;
      return;
    }
    ...
    var rows = cmp.map(function (r) {
      ...
      var sign = r.diff > 0 ? '+' : '';
      return '<tr><td>' + esc(coffeeName(r.sku)) + '</td>' +
  ...
  function renderSettings() {
    ...
    host.innerHTML = catalog.map(function (c, i) {
      return '<div class="catalog-row" data-i="' + i + '">' +
        '<div class="cr-main"><div>' + esc(c.name) + '</div>' +
        '<div class="cr-sku">' + esc(c.sku) +
        (c.aliases && c.aliases.length ? ' · alias: ' + esc(c.aliases.join(', ')) : '') + '</div></div>' +
        '<button class="icon-btn cr-alias" title="Alias">✏️</button>' +
        '<button class="icon-btn cr-del" title="Borrar">✕</button>' +
        '</div>';
    }).join('');
    ...
    lh.innerHTML = keys.length ? keys.map(function (k) {
      ...
      return '<div class="legacy-row" data-k="' + esc(k) + '">' +
        ...
        '<span>' + esc(coffeeName(m.sku)) + ' · ' + (m.lbs ? E.fmtLbs(m.lbs) + ' lb' : 'pregunta lbs') + '</span>' +
        '<button class="icon-btn lg-del">✕</button></div>';
  ...
  function renderBoxRules() {
    ...
    host.innerHTML = settings.boxRules.map(function (r, i) {
      return '<div class="rule-row" data-i="' + i + '">' +
        ...
        '<button class="icon-btn rule-del">✕</button></div>';
    }).join('');
  ...
  window.BCApp = {
    ...
    exitScan: exitScan,
    showView: showView,
    getState: function () { return { orders: orders, scans: scans, catalog: catalog, session: scanSession }; }
  };
})();
